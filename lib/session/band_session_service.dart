...
class BandState {
  ...
  BandState copyWith({
    ...
    return BandState(
      ...
      isNewBp: isNewBp ?? this.isNewBp,
      isNewSpo2: isNewSpo2 ?? this.isNewSpo2,
    );
  }
}
...
class BandSessionService {
  ...
  BandTimingLogger? _timingLogger;
  // ── End timing ──────────────────────────────────────────────────────────────
    ...
    switch (event) {
      ...
      case RealtimeEvent(:final data):
        ...
        if (_firstHr && next.hr > 0) {
          _firstHr = false;
          debugPrint(
              '[$deviceId] ⏱ First HR (${next.hr} bpm) received ${sinceConnect}ms after connect');
          _timingLogger?.log('first_hr',
        ...
        if (_firstSpo2 && next.spo2 > 0) {
          _firstSpo2 = false;
          debugPrint(
              '[$deviceId] ⏱ First SpO2 (${next.spo2}%) received ${sinceConnect}ms after connect');
          _timingLogger?.log('first_spo2',
              sinceConnectMs: sinceConnect, intervalMs: null, spo2: next.spo2);
        }
        if (_firstTemp && next.tempC > 0) {
          _firstTemp = false;
          debugPrint(
              '[$deviceId] ⏱ First Temp (${next.tempC}°C) received ${sinceConnect}ms after connect');
          _timingLogger?.log('first_temp',
              sinceConnectMs: sinceConnect,
              intervalMs: null,
              tempC: next.tempC);
        }
        ...
        if (_lastVitalsTime != null) {
          ...
          debugPrint(
              '[$deviceId] ⏱ First VITALS update at ${sinceConnect}ms after connect');
          _timingLogger?.log('first_vitals',
              sinceConnectMs: sinceConnect,
              intervalMs: null,
              hr: next.hr,
              spo2: next.spo2,
      ...
      case ManualMeasurementEvent(
        ...
        final stressVal = stress > 0 ? stress : _state.stress;
        final sysVal = systolic > 0 ? systolic : _state.systolic;
        final diaVal = diastolic > 0 ? diastolic : _state.diastolic;
      ...
      case BatteryEvent(:final percent):
        ...
        if (_firstBattery) {
          ...
          final sinceConnect = _connectTime != null
              ...
              : -1;
          debugPrint(
              '[$deviceId] ⏱ First Battery ($percent%) received ${sinceConnect}ms after connect');
      ...
      case UnknownEvent(:final dataType, :final raw):
        debugPrint(
            '[$deviceId] Unhandled event type=0x${dataType.toRadixString(16)} raw=$raw');
    }
  }
  ...
  Future<void> _runIngest() async {
    try {
      ...
      if (_state.isNewBp || _state.isNewSpo2) {
        _emit(_state.copyWith(isNewBp: false, isNewSpo2: false));
      }
    } catch (e) {
